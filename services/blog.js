import { state, editingPostId, setEditingPostId } from './store.js';
import { apiPost, apiPatch, apiDelete } from './api.js';
import { showNotif, openModal, closeModal, setVal, getVal } from './ui.js';
import { populateBlog } from './populate.js';

export function nuevoBlog() {
  ...
  setVal('b-extracto', '');
  setVal('b-img',      '');
  document.getElementById('b-fecha').value   = new Date().toISOString().split('T')[0];
...
export async function saveBlog() {
  ...
  if (!titulo) { showNotif('El título es requerido', 'error'); return; }

  const item = {
    titulo,
    categoria: document.getElementById('b-categoria').value,
    estado:    document.getElementById('b-estado').value,
    fecha:     document.getElementById('b-fecha').value || new Date().toISOString().split('T')[0],
  ...
  try {
    if (editingPostId) {
      ...
      if (idx > -1) state.blog.posts[idx] = { ...state.blog.posts[idx], ...item };
    } else {
